...
// MAKE A CURSOR TO LOOP THROUGH LIST
    Node*  cur = head_;
      while(cur != NULL){

// IF DATA IN CURSOR BIGGER THAN NEW DATA do something!
        if (cur->data > newNode->data){
// IF WE ARE AT HEAD JUST ADD TO FRONT
          if (cur == head_){
            cur->prev = newNode;
            newNode->next = cur;
            head_ = newNode;
...
// ELSE MAKE TWO NEW NODES ONE POINTING TO CURRENT POSITION ONE TO Previous
// MAKE SURE EVERYTHANG IS LINKED TO THE NEW NODE BEFORE INSTERTING CUR=NEWNODE
          else{

            Node* oldHead = cur;
            Node* newoldHead = cur->prev;
            oldHead->prev = newNode;
            newoldHead->next = newNode;
            newNode->next = oldHead;
            newNode->prev = newoldHead;
            cur = newNode;
            break;
        }
        }
    ...
    // IF WE FINISH LOOPING AND CURSOR POITNS TO NULL MAKE A NEW TAIL WITH NEW DATA
    if (cur == NULL){
      Node* oldTail = tail_;
...
// check for sizes
  if (left.size()>right.size()){
    // als rechter nul is dan return meteen linker luist
    if (right.size() ==0){
      merged = left;
      return merged;
    }
