...
static void treeFactory(GenericTree<int>& tree) {
  ...
  auto cur2 = tree.getRootPtr();
  // std::cout << cur->data << std::endl;
  tree.deleteSubtree(cur2);
  // std::cout << cur->data << std::endl;
  tree.createRoot(4);
  auto cur = tree.getRootPtr();
  auto node8 = cur->addChild(8);
  node8->addChild(16)->addChild(42);
  node8->addChild(23);
  auto node15 = cur->addChild(15);
  cur2 = nullptr;
...
// MAKE QUEUES FOR NODES TO EXPLORE (WRITE DATA)
// NODES TO TRAVEL(LOOP in oRDER TO GET NODESTOEXPLORe)
// NODES PRETRAVEL LOOP IN ORDER TO GET RIGHT ORDER OF NODES TO TRAVEL
// THE LAST ONE IS UNNECESSARY AS CAN BE SEEN IN MY UNCOMMENTED CODE
  std::queue<TreeNode*> nodesToExplore;
  std::queue<TreeNode*> nodesToTravel;
  std::queue<TreeNode*> nodesPreTravel;
  // Now you can refer to a pointer to a TreeNode in this function like this.
  // TreeNode* someTreeNodePointer = nullptr;
  TreeNode* cur = tree.getRootPtr();

  // This is the results vector you need to fill.
  std::vector<T> results;
  results.push_back(cur->data);
  nodesPreTravel.push(cur);

//   while(nodesPreTravel.size()!=0){
//     nodesToTravel.push(nodesPreTravel.front());
//     nodesPreTravel.pop();
//   for (unsigned int i=0;nodesToTravel.size();i++){
//
//
//   for (TreeNode* childPtr : nodesToTravel.front()->childrenPtrs) {
//     if (!childPtr){
//       continue;
//     }
//     nodesToExplore.push(childPtr);
//     nodesPreTravel.push(childPtr);
//
...
nodesToTravel.push(cur);

// KEEP LOOPING WHILE THERE IS SOMETHING IN NODES TO TRAVEL QUEUE
for (unsigned int i=0;i<nodesToTravel.size();i++){
...
for (TreeNode* childPtr : nodesToTravel.front()->childrenPtrs) {
  ...
  nodesToExplore.push(childPtr);
  nodesToTravel.push(childPtr);

  results.push_back(childPtr->data);
}
nodesToTravel.pop();
// THIS IS SO WE KEEP LOOPING BY RESETTING I
// IF THERE IS NOTHING IN NODES TO TRAVel size == 0 and i== size nodestoTRAVEL
// making our program stop
i = 0;
}
...
// std::vector<T> resultstracker(N* subtreeRoot) {
//
// }
