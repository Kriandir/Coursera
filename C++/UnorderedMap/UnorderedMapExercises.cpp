...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  for (unsigned i=0; i < words.size(); i++) {
    ...
    if ( wordcount_map.count(words[i]) > 0 ) {
      wordcount_map.at(words[i]) +=1;
    }
    else{
      wordcount_map.insert(std::pair<std::string,int>(words[i],1));
  ...
  // std::cout << wordcount_map.at("bandersnatch") << std::endl;
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  if ( wordcount_map.count(key) > 0 ) {
    fallbackVal = wordcount_map.at(key);
  }
